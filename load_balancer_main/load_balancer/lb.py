...
from lb_input import get_usage_percent, get_network_bandwidth, get_application_table
# from load_balancer_main.load_balancer.lb_input import get_usage_percent, get_network_bandwidth, get_application_table
...
class LoadBalancerNode:
    ...
    def image_callback(self, ros_image):
        ...
        if self.single_shot and self.request_sent:
            return  # BLOCK all further frames
    ...
    def handle_frame(self, frame, ros_image):
        ...
        self.start_pub.publish(current_time)
        # if current_time - self.last_processing_time < self.min_processing_interval:
        #     return
        # self.last_processing_time = current_time

        # rospy.loginfo(f"[{self.trace_id}] CPU={cpu_usage:.1f}% GPU={gpu_usage:.1f}% " f"UL={upload_speed:.2f} DL={download_speed:.2f}")
        ...
        try:
            ...
            for app in applications:
                ...
                # rospy.loginfo(f"[{self.trace_id}] App={app} " f"Latency={latency_sensitivity} Accuracy={accuracy_priority}")
                if r is None:
                ...
                rospy.loginfo(f"[{self.trace_id}] Decision → {processing_location.upper()}")
    ...
    def decide_processing_location(self, latency_sensitivity, accuracy_priority, 
        ...
        # print(bandwidth_sufficient, bandwidth_low, onboard_resources_sufficient)
        # print(upload_speed, download_speed)
        
        # Main decision logic following the workflow
        if latency_sensitivity == "high":
            # High latency sensitivity path
            # rospy.loginfo("High latency sensitivity application, checking onboard resources.")
            if onboard_resources_sufficient:
                # rospy.loginfo("Onboard resources sufficient for high latency sensitivity.")
                return "onboard"
            else:
                # Resources insufficient
                # rospy.loginfo("Onboard resources insufficient for high latency sensitivity, checking bandwidth.")
                if bandwidth_low:
                    # rospy.loginfo("No Bandwidth, processing onboard despite high latency sensitivity.")
                    return "onboard"
                elif bandwidth_sufficient:
                    # rospy.loginfo("Bandwidth sufficient for offloading high latency sensitivity application.")
                    return "offboard"
                else:
                    # Bandwidth is low, check accuracy priority
                    if accuracy_priority == "high":
                        # rospy.loginfo("Accuracy priority high, processing onboard despite low bandwidth.")
                        return "onboard"
                    else:
                        # rospy.loginfo("Lowering resolution due to low bandwidth and lower accuracy priority.")
                        return "lower_resolution"
                        
        else:  # latency_sensitivity is "low" or "medium"
            # Low latency sensitivity path
            # rospy.loginfo("Low latency sensitivity application, checking accuracy priority.")
            if accuracy_priority == "high":
                # rospy.loginfo("Accuracy priority high for low latency sensitivity, checking bandwidth.")
                if bandwidth_low:
                    # rospy.loginfo("No Bandwidth, processing onboard despite high latency sensitivity.")
                    return "onboard"
                elif bandwidth_sufficient:
                    # rospy.loginfo("Bandwidth sufficient for offloading with high accuracy priority and low latency sensitivity.")
                    return "offboard"
                else:
                    # rospy.loginfo("Bandwidth low, processing onboard due to high accuracy priority and low latency sensitivity..")
                    return "onboard"
            else:  # accuracy_priority is "low" or "medium"
                # rospy.loginfo("Accuracy priority low for low latency sensitivity, checking bandwidth.")
                if bandwidth_sufficient:
                    # rospy.loginfo("Bandwidth sufficient for offloading with low accuracy priority and low latency sensitivity.")
                    return "offboard"
                else:
                    # rospy.loginfo("Lowering resolution due to low bandwidth and low accuracy priority with low latency sensitivity.")
                    return "lower_resolution"
    ...
    def forward_to_edge(self, frame, app):
        ...
        try:
            ...
            rospy.loginfo(f"[{self.trace_id}] Sent request to EDGE ({app})")
    ...
    def process_edge_response(self, response, app):
        ...
        rospy.loginfo(f"[{self.trace_id}] Response received from EDGE ({app})")
                ...
                if 'Traffic_detection_model' in result:
                    ...
                    if isinstance(traffic_data, str):
                        try:
                            ...
                            return
                    # Convert to the same format as OBD_model detections
                    if traffic_data.get('success', False):
                        ...
                        # Create detections in the same format as OBD_model
                        detections = []
                        ...
                        rospy.logwarn("Traffic detection was not successful")
                        self.traffic_sign_results = []
                    
                # rospy.loginfo(f"Successfully processed edge response for {app}")
                rospy.loginfo(f"[{self.trace_id}] Parsed {app} results successfully")
    ...
    def process_lane_data(self, lane_data):
        ...
        try:
            ...
            # rospy.loginfo(f"Lane data processed from edge - X: {self.lane_data['lane']['x']}, Angle: {self.lane_data['lane']['angle']}")
    ...
    def forward_to_onboard(self, app, ros_image):
        ...
        rospy.loginfo(f"[{self.trace_id}] Forwarded to ONBOARD → {app}")
        try:
