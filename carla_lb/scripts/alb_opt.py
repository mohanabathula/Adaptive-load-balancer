...
Author: Vidya Vepoori, Yash 
Date: 2026-02-18
Version: 2.0 TRUE ALB + depth estimate optimized 
"""
...
CSV_FILE = "/media/root/b9f388cb-81d8-4121-93fb-ad514e324552/vidyav/main/alb_opt.csv"
...
class AsyncCSVWriter:
    """Non-blocking CSV writer using background thread."""
    ...
    def _write_loop(self):
        ...
        while self.running:
            try:
                ...
                # Flush batch when queue is empty or batch is large
    ...
    def close(self):
        """Close writer and wait for queue to drain."""
        self.running = False
...
class TrueAdaptiveLoadBalancerV5:
    ...
    def __init__(self):
        ...
        # ===== CSV Logging with GPU tracking + optimization metrics =====
        ...
        csv_header = [
            ...
            "distance_calc_ms", "csv_write_ms",
            "vehicle_speed_kmh", "target_distance_m", "min_distance_m", 
            "num_detections", "collision_detected", "collision_actor_id"
        ]
        self.async_csv_writer = AsyncCSVWriter(CSV_FILE, csv_header)
        print(f"{Color.GREEN}[INIT] ? Async CSV writer initialized (GPU + optimization tracking){Color.RESET}\n")
    ...
    def shutdown(self, signum=None, frame=None):
        ...
        if hasattr(self, 'async_csv_writer'):
            self.async_csv_writer.close()
    ...
    def handle_frame(self, frame, frame_id, frame_rx_time):
        ...
        try:
            ...
            onboard_distance_calc_ms = 0.0
            
            if onboard_apps:
                print(f"\n{Color.GREEN}[ONBOARD] Processing {len(onboard_apps)} apps with GPU tracking...{Color.RESET}")
                onboard_inference_ms, onboard_gpu_compute_ms, onboard_gpu_idle_ms, onboard_distance_calc_ms = \
            ...
            # ============================================================
            # STEP 7: Log to CSV (one row per app) with GPU + optimization metrics
            # ============================================================
            for app in ACTIVE_APPLICATIONS:
                ...
                if location == "onboard":
                    ...
                    distance_calc_ms = onboard_distance_calc_ms / max(1, len(onboard_apps))
                    ...
                    distance_calc_ms = 0.0  # Edge doesn't calculate distance locally
                else:
                    app_inference_ms = 0.0
                    gpu_compute_ms = 0.0
                    gpu_idle_ms = 0.0
                    distance_calc_ms = 0.0
                
                # Time the CSV write operation
                ...
                self.log_to_csv(
                    ...
                    distance_calc_ms=distance_calc_ms,
                    csv_write_ms=(time.time() - csv_write_start) * 1000.0,
                    speed_kmh=speed_kmh,
    ...
    def process_onboard_batch_with_gpu_tracking(self, frame, frame_id, apps):
        ...
        try:
            ...
            if need_object_model:
                ...
                # Extract vehicle/pedestrian detections with distance timing
                car_count = 0
                ...
                distance_calc_ms = (time.time() - distance_calc_start) * 1000.0
            ...
            print(f"{Color.GREEN}[ONBOARD] Total: {total_inference_ms:.2f}ms | "
                  f"GPU Compute: {total_gpu_compute_ms:.2f}ms | GPU Idle: {gpu_idle_ms:.2f}ms | "
                  f"Distance Calc: {distance_calc_ms:.2f}ms{Color.RESET}")
            
            return total_inference_ms, total_gpu_compute_ms, gpu_idle_ms, distance_calc_ms
    ...
    def _calculate_distance(self, bbox):
        """Optimized distance calculation using mean instead of median (O(k) vs O(k log k))."""
        ...
        try:
            ...
            if x2 > x1 and y2 > y1:
                ...
                if patch.size > 0:
                    return float(np.mean(patch))  # OPTIMIZED: mean is O(k), median is O(k log k)
        except:
            pass
        return None
    ...
    def log_to_csv(self, timestamp, frame_id, app_type, location, cpu, gpu, ram,
                   ...
                   distance_calc_ms, csv_write_ms, speed_kmh, target_dist, min_dist, 
                   num_detections, collision, collision_id):
        """Non-blocking CSV write using async writer."""
        
        self.async_csv_writer.write_row([
            timestamp, frame_id, app_type, location, cpu, gpu, ram, tx_mbps, rx_mbps,
            edge_available, app_inference_ms, gpu_compute_ms, gpu_idle_ms,
            resource_check_ms, decision_ms, total_frame_ms, distance_calc_ms, csv_write_ms,
            speed_kmh,
